...
{
   IIC_Start();
   Write_IIC_Byte(0x78);            //Slave address,SA0=0£» ÉèÖÃÄÚ´æÑ°Ö·Ä£Ê½
	IIC_Wait_Ack();	
   Write_IIC_Byte(0x00);			//write commandÁÐµØÖ·Ä£Ê½£¨01ÐÐµØÖ·Ä£Ê½ £»10 Ò³ µØÖ·Ä£Ê½ £»Ä¬ÈÏ Ò³µØÖ·Ä£Ê½
	IIC_Wait_Ack();	
...
{  
	...
	{  
		...
		OLED_WR_Byte (0x00,OLED_CMD);      //ÉèÖÃÏÔÊ¾Î»ÖÃ¡ªÁÐµÍµØÖ·
		OLED_WR_Byte (0x10,OLED_CMD);      //ÉèÖÃÏÔÊ¾Î»ÖÃ¡ªÁÐ¸ßµØÖ·   
		for(n=0;n<128;n++)OLED_WR_Byte(1,OLED_DATA); //nÊÇÁÐ
	} //¸üÐÂÏÔÊ¾
}
...
{      	
	unsigned char c=0,i=0;	
		...
		if(Char_Size ==16)
			{
			OLED_Set_Pos(x,y);	
			for(i=0;i<8;i++)//ÕâÊ±iÊÇÐÐ
			OLED_WR_Byte(F8X16[c*16+i],OLED_DATA);//£¨cmd  £¬dat 1£©
			OLED_Set_Pos(x,y+1);
			for(i=0;i<8;i++)
...
{ 	
 	...
 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOG, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//ËÙ¶È50MHz
 	...
 	GPIO_SetBits(GPIOB,GPIO_Pin_6|GPIO_Pin_7);	//PD3,PD6 Êä³ö¸ß
